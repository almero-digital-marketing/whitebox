...
#
# Integrations live OUTSIDE the monorepo (default: ../whitebox-pro-integrations,
# override with WB_INTEGRATIONS_DIR), each in its own repo. This wires them into
# the monorepo's node_modules so `import { mailgun } from 'whitebox-pro-mail-mailgun'`
# (etc.) resolves. Run it after `npm install`. Idempotent; skips anything not
# present on disk.
#
# Two cases:
#   • adnetwork packages depend on the in-monorepo `whitebox-pro-adnetworks` kernel,
...
#     those, skipping the unpublished `whitebox-pro-*` peer deps.
set -uo pipefail
...
for dir in "$INTEGRATIONS_DIR"/*/; do
  ...
  if node -e "process.exit((require('$dir/package.json').dependencies||{})['$KERNEL']?0:1)"; then
    ...
    # the unpublished whitebox-pro-* peer deps. We verify a declared dep actually
