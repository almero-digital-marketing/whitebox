...
Browser client library for [whitebox-server](../whitebox-server). Sits on customer websites, manages passport/session identity, ships engagement events, submits contact forms, and tracks conversions.
...
`socket.io-client` ships as a dependency — whitebox-server uses Socket.IO and the client must match.
